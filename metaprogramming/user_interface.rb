class UserInterface
  ...
  def info
    puts ['Введите 1 для создания станций',
          ...
          'Введите 12 для теста accesors.rb']
    user_response
  end
  ...
  def methods_arr
    @methods_arr = { 1 => :create_stations, 2 => :create_trains,
                     ...
                     11 => :loading_trains, 12 => :check_accessors }
  end
  ...
  def check_accessors
    puts 'IRB Для проверки модуля accessrs в классе Train c переменной type'
    puts 'Для возвращения в меню введите exit'
    line_num = 0
    @input = ""
    loop do
      print "#{line_num += 1}?: "
